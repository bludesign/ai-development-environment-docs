...
**Skip pending clients** appears when that happens: it marks the stalled agents' items skipped and lets the run proceed with the Macs that did report. Skipped agents are excluded from deployment entirely, so their copies are left untouched rather than being treated as deletions.

...
<AccordionGroup>
  ...
  <Accordion title="Builds" icon="hammer">
    **Start Build** picks one saved configuration and a destination. The selected configuration names its source path, scheme, build configuration, and the agent the build will run on — the one that owns the codebase — so you can tell where the work is going before you start it.
    ...
    A successful **Latest build** badge is a direct keyboard-accessible link to the [build details](/builds/build-detail) page. Rebuild and run actions remain available beside it. Builds that produced a runnable app use the detailed device picker described on the [Builds page](/builds/builds#running-on-devices).
  </Accordion>

...
The **Build History** card loads the latest 50 builds for this worktree and appends older records with **Load more**. Select a row to open its immutable build details. Select any **Out of date** badge to open **Rebuild** without leaving this surface. **Copy path** in the worktree actions copies the complete folder path. **Rebuild** queues a new build without leaving the page, and a successful runnable build adds **Run** with the shared detailed agent-and-device picker. It defaults to the build agent, lazily loads devices when you select another eligible Mac, and transfers the app through the control plane when the active agent differs from the builder.

...
Endpoint detail shows history scoped to that endpoint. `/sse/history` combines all endpoints and adds a prominent endpoint filter and endpoint-name column. Switch between **Streams** and **Events**, then use text, glob, or regular-expression search; quick and advanced filters; saved filters; configurable columns and presets; pagination; live updates; selection; and clear/delete controls.

Stream details retain the original and effective request, duplicate header values, the mode/configuration snapshot, upstream and emitted response headers, source and emitted events, IDs, timings, truncation, and errors. Export matching data as CSV, Markdown, or PDF. The iOS app provides the same stream/event toggle, filtering, details, and native share sheet.

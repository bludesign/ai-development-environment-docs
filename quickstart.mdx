...
There are three ways to install AI Development Environment. Pick one, then enroll a control agent so the dashboard has a Mac to work with.
...
| [From source](#option-3-from-source) | Developing the app itself | `3000` | Database |

## Prerequisites

- macOS for the control agent, iOS builds, signing, and Keychain credential storage. The control plane itself also runs on Linux.
...
The dashboard needs at least one enrolled Mac before it can create worktrees, run commands, or start builds. The agent connects outbound only — managed Macs never expose a listening port.
...
<Steps>
  <Step title="Install the agent">
    ...
    The npm install in Option 2 already includes `@ai-development-environment/control-agent`, and `npm run dev:all` runs its own development agent, so you can skip this step in those cases.
  </Step>
  ...
  <Step title="Run the enrollment on the target Mac">
    ```bash
...
Long-lived Jira, GitHub, cache-server, MCP, iOS signing, App Store Connect, and APNs secrets go through a server-only credential service. Choose a backend with `CREDENTIAL_STORAGE_TYPE`.
...
<Tabs>
  ...
  <Tab title="Vault">
    ...
    The app needs data read/write plus permanent metadata deletion, and never `LIST`:

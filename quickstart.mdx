...
<Steps>
  ...
  <Step title="Know where things live">
    ...
    Add `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL=http://127.0.0.1:3090`, and `AUTH_MODE=password` to that file. Then start the service:
...
<Steps>
  ...
  <Step title="Start the server">
    ```bash
    BETTER_AUTH_SECRET="replace-with-your-generated-secret" \
...
docker run --detach \
  ...
  --publish 3090:3090 \
  --publish 3091:3091 \
  --env BETTER_AUTH_SECRET="replace-with-your-generated-secret" \
  --env BETTER_AUTH_URL="http://localhost:3090" \
...
<Steps>
  ...
  <Step title="Configure the database">
    Copy the example environment file and adjust `DATABASE_URL` if you want the SQLite file somewhere other than `prisma/dev.db`. Only `file:` URLs are accepted. Set `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL=http://127.0.0.1:3000`, and `AUTH_MODE=password` too.

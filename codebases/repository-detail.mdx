...
A repository is usually checked out on more than one Mac, and each checkout can carry several worktrees. Putting build configurations, skill groups, and retry rules on the repository means a new checkout inherits them the moment it is registered — there is nothing to copy per machine, and no way for two Macs to drift apart.

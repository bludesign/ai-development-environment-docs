...
<Steps>
  ...
  <Step title="Answer questions" icon="circle-question-mark">
    A question batch appears inline as **Answer needed**, with options, optional multi-select, and a free-text field where the run allows one. Submitting resumes the run. Answering also re-takes the worktree lease if the Session has lost it.
  </Step>
  ...
  <Step title="Pause or cancel" icon="circle-stop">
    Pausing holds the run and its lease. Cancelling ends it and releases the lease, leaving the worktree in whatever state the run reached.
  </Step>
</Steps>
...
<Warning>
  Starting a resumed follow-up from a paused Session cancels the parent and atomically transfers its worktree lease to the child. The page asks you to confirm — the parent will not survive it.
</Warning>
...
<Columns cols={2}>
  ...
  <Card title="Worktree details" icon="git-branch" href="/worktrees/worktree-detail">
    The checkout this Session holds a lease on.
  </Card>

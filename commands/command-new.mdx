...
| **Worktree in selected repositories** | Any worktree belonging to one or more selected repositories | You pick one worktree |

When you choose **Worktree in selected repositories**, select at least one repository from the checklist. You can select any number of repositories, including repositories hosted by different agents. At launch, the eligible-target picker shows worktrees from the complete allow-list. The concrete worktree you choose determines which agent executes the command, and each launch creates one run.

<Tip>
  Prefer the narrowest scope that still fits. A **Specific agent home** command cannot be launched against the wrong machine by accident, and a selected-repository worktree command cannot wander into an unrelated checkout.
</Tip>
...
**Show this command as a quick action** puts the command on the pages of the things it targets: agent-home commands on agent pages, worktree commands on worktree pages. For **Worktree in selected repositories**, the button appears on worktrees from every selected repository.

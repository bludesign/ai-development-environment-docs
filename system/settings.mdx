...
Every secret entered here is written through the server-only credential service. Values are stored server-side, never returned to the browser, and appear on the [Credentials](/system/credentials) page as metadata only.
...
The chosen edition has to be installed on the Mac that *hosts* the worktree, not on the machine running your browser — the action runs on the agent.

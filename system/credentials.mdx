...
| --- | --- |
| **APP_SECRET is invalid** | `APP_SECRET` is missing, or is not base64 or hex of exactly 32 bytes. Database storage cannot open without it |
| **APP_SECRET does not match stored credentials** | The configured root cannot decrypt the existing rows. Restore the original value, or set it as `APP_SECRET_PREVIOUS` so the rows are re-encrypted on the next start |

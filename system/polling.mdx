...
The page has two cards. **Server operations** are loops running inside the server process. **Agent operations** are loops running on each enrolled Mac — one row per agent per operation, with the agent name linking to its [agent detail](/agents/agent-detail) page.
...
<Steps>
  <Step title="Check the runtime" icon="server">
    A failing server operation affects everyone. A failing agent operation affects only that Mac — and if every row for one agent is stale, the problem is the connection, not the operations.
  </Step>

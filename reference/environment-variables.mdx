...
<ParamField path="AIDE_SERVER_HOSTNAME" type="string" default="127.0.0.1">
  Interface the HTTP server binds to when you use the installed npm launcher or Docker image. Set it to `0.0.0.0` only when a container or reverse proxy must reach the listener.

  The launcher intentionally ignores the ambient `HOSTNAME` variable because shells and container runtimes commonly populate it with a network-reachable machine name. A source checkout started directly with `npm start`, and the Homebrew service's explicit environment file, still use Next.js's `HOSTNAME` variable.
</ParamField>

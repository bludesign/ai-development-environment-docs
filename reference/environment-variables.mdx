...
The remaining variables apply only when `AUTH_MODE` is `oidc` or `both`. Supply either a discovery URL **or** all three explicit endpoint URLs — not both. PKCE is always enabled. Issuer validation defaults to `false` and can be enabled with the setting below. See [Authentication](/reference/authentication) for the full workflow.

...
| `CONTROL_AGENT_DSYM_CACHE` | Where a macOS agent caches DWARF files for crash symbolication, up to 10 GiB | `~/Library/Caches/control-agent/dsym-cache` |
...
| `ARTIFACT_TRANSFER_DIRECTORY` | Durable staging for resumable cross-agent uploads; use persistent local storage          | System temporary directory |
| `ARTIFACT_TRANSFER_MAX_BYTES` | Maximum accepted size of one transferred package                                         | 20 GiB                     |
| `RUN_DATA_DIRECTORY`          | Where run attachments are stored                                                        | Under the system data path |
| `CRASH_DATA_DIRECTORY`        | Where crash reports and extracted dSYMs are stored; back it up with the database          | `crash-data` next to the SQLite database |
| `DSYM_UPLOAD_MAX_BYTES`       | Largest dSYM zip accepted by one `POST /api/dsyms` request; resumable uploads allow 20 GiB | 2 GiB                      |

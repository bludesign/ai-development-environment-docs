...
| dSYM uploads (`/api/dsyms`)      | Full                   | Full         | No                    | No                                         |
| Crash downloads (`/api/crash-files/*`) | Full             | Full         | No                    | No                                         |
| Crash uploads (`/api/public/crashes`) | Recorded         | Recorded     | No                    | Yes, rate limited                          |
...
- `/api/public/*`, whose webhook signatures, short-lived artifact tokens, and crash upload rate limits remain enforced
- `/api/openapi.json`
- `/api/telemetry/analytics-events` and `/api/telemetry/console-logs`
- `/api/dsyms` and `/api/dsyms/uploads/*`, which CI reaches with an `X-API-Key` — the route checks the key itself

...
<Note>
  Automatic development enrollment can target a local or remote development
  control plane. Set `CONTROL_AGENT_DEV_SERVER` and, when necessary,
  `CONTROL_AGENT_DEV_WEBSOCKET_SERVER` to its reachable addresses. The first
  enrollment still requires a valid one-time token; using a remote address does
  not bypass the enrollment flow.
</Note>
...
For agent-only development, `CONTROL_AGENT_DEV_SERVER`, `CONTROL_AGENT_DEV_WEBSOCKET_SERVER`, and `CONTROL_AGENT_DEV_CONFIG` override the control-plane endpoints and the dedicated credential path — useful for pointing a watch-mode agent at a server you started separately, including one on another host.
...
| `npm run agent:dev` | Starts only the watch-mode development agent, waiting for its configured development control plane. |

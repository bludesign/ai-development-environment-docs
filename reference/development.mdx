...
Then copy the example environment file. The only variable a source checkout needs is `DATABASE_URL`; everything else in the file is optional and commented out.
...
<Note>
  Automatic development enrollment refuses non-loopback server addresses. It is a convenience for `127.0.0.1`, not a way to skip the enrollment-token flow against a real server.
</Note>
...
│   └── control-agent/    The TypeScript control agent that runs on each Mac
...
<Tip>
  Services are the seam worth learning first. GraphQL resolvers, REST route handlers, and MCP tools are three thin adapters over the same `src/services/` layer, which is why a capability added once shows up in all three.
</Tip>
...
The agent is a client, not a server: it dials the control plane's GraphQL WebSocket outbound and executes jobs it is handed. Nothing on a managed Mac listens for inbound connections. It bundles the Claude Agent SDK, the Codex SDK, and the OpenCode SDK, which is how a Session actually runs a model on the target machine.

<Warning>
  Changing `packages/agent-contract` changes a wire contract. A server and an agent built from different revisions can disagree about job payloads, so rebuild and restart both after touching it.
</Warning>
...
<Note>
  The mock database's credentials are encrypted with a fixed key that the capture server passes back in as `CREDENTIAL_ENCRYPTION_KEY`. If those two ever diverge, the app rewrites every credential row on first use and the `VACUUM` that follows locks the database mid-capture.
</Note>

...
| [REST](#rest-endpoints) | `/api/codebases`, `/api/telemetry/*`, `/api/public/crashes`, `/api/dsyms`, `/api/ios/apns-devices` | Narrow integrations with route-specific security   |
...
Apps post crash reports and CI posts debug symbols here. See [Crashes](/debugging/crashes#send-crashes-from-your-app) and [dSYMs](/debugging/dsyms#upload-from-ci) for full examples. GitHub Actions workflows can use the [Upload dSYMs action](/debugging/upload-dsyms-action), which runs the resumable upload.

| Endpoint | Credential | Accepts |
| --- | --- | --- |
| `POST /api/public/crashes` | None required; `X-API-Key` or a session is recorded when sent | One `.ips`, `.crash`, or MetricKit JSON report as the raw body, up to 5 MiB after optional gzip decoding |
| `POST /api/dsyms` | `X-API-Key` or a session | A zip of `.dSYM` bundles as the `file` part of `multipart/form-data`, or a raw `application/zip` body, with optional `projectName`, `buildId`, and `url`; up to `DSYM_UPLOAD_MAX_BYTES` (2 GiB by default) |
| `POST /api/dsyms/uploads` | `X-API-Key` or a session | Starts a resumable upload of up to 20 GiB |
| `HEAD` / `PATCH /api/dsyms/uploads/{id}` | The credential that started it | Reports or appends at `Upload-Offset`, 16 MiB per chunk |
| `POST /api/dsyms/uploads/{id}/complete` | The credential that started it | Verifies the optional `sha256` and indexes the zip |
| `GET /api/crash-files/reports/{id}` | `X-API-Key` or a session | The original report, or `?variant=symbolicated` for the symbolicated text |
| `GET /api/crash-files/dsyms/{id}` | `X-API-Key` or a session | One dSYM bundle as a zip |

Anonymous crash uploads are limited to 30 a minute per address and answer `202` without storing anything while crash collection is off. A bad credential is refused with `401` rather than treated as anonymous.
...
| Endpoint                | Returns                                                                      |
| ----------------------- | ---------------------------------------------------------------------------- |
| `GET /api/openapi.json` | An OpenAPI 3.1 document covering the codebase, telemetry, APNs, crash, and dSYM endpoints |
...
| `GET /api/agent/dsyms/{id}/dwarf`                                    | A macOS agent fetching a DWARF file for a crash it is symbolicating |
...
Everything under `/api/public/*` is intentionally unauthenticated: iOS enrollment callbacks, short-lived build artifact and over-the-air manifest downloads, rate-limited crash report uploads, and the three signature-verified webhooks. [Hosting and networking](/reference/hosting) lists the full path set and explains how to expose exactly that namespace and nothing else.

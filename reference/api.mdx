...
| [REST](#rest-endpoints) | `/api/codebases`, `/api/telemetry/*`, `/api/crashes`, `/api/dsyms`, `/api/ios/apns-devices` | Narrow integrations with route-specific security   |
...
| --- | --- | --- |
| `POST /api/crashes` | None required; `X-API-Key` or a session is recorded when sent | One `.ips`, `.crash`, or MetricKit JSON report as the raw body, up to 5 MiB after optional gzip decoding |

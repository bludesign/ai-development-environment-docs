...
services:
  server:
    ...
    environment:
      ...
      BETTER_AUTH_URL: ${BETTER_AUTH_URL:-http://localhost:3090}
...
printf 'BETTER_AUTH_URL=http://localhost:3090\n' >> .env
```

Keep `.env` out of source control and back it up securely. Use the public HTTPS origin for `BETTER_AUTH_URL` behind a reverse proxy. Add the provider variables from [Authentication](/reference/authentication#configure-oauthoidc) when using `AUTH_MODE=oidc` or `both`.

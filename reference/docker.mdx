...
The image supports the same environment variables as the npm server, including Better Auth and OIDC settings, `PORT`, `AIDE_SERVER_HOSTNAME`, `AGENT_WS_HOSTNAME`, `AGENT_WS_PORT`, `DATABASE_URL`, credential-storage settings, and Vault settings. It applies pending Prisma migrations before starting. The image sets `AIDE_SERVER_HOSTNAME=0.0.0.0`; an ambient container `HOSTNAME` never controls the bind address.

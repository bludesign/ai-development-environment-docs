...
<Steps>
  <Step title="Pull and start the server">
    ...
    Open `http://localhost:3090`, go to **Agents**, and create a one-time enrollment token.
  </Step>
...
<Note>
  The agent currently reports capacity for its container root filesystem. A bind-mounted disk is available to jobs and repositories, but its capacity is not included in the agent's disk-usage field.
</Note>
...
The image supports the same environment variables as the npm server, including `PORT`, `HOSTNAME`, `AGENT_WS_HOSTNAME`, `AGENT_WS_PORT`, `DATABASE_URL`, credential-storage settings, and Vault settings. It applies pending Prisma migrations before starting.

...
Everything the control plane knows lives in one SQLite file: agents and their jobs, codebases and worktrees, builds and their logs, AI runs, workflows, telemetry, the GitHub and Jira caches, and — depending on the credential backend — your stored secrets. There is no separate cache, queue, or object store to operate.
...
<Warning>
  The database file holds the profile signer's private key, device UDIDs, IP history, any App Store Connect `.p8` key, and — with the `database` credential backend — every stored credential. Treat backups as sensitive as the original.
</Warning>

A backup is only restorable together with `CREDENTIAL_ENCRYPTION_KEY`. Encrypted credential rows cannot be read without it, and key rotation is not currently supported, so store the key separately from the backups but do not lose it. See [credential storage](/quickstart#set-up-credential-storage).

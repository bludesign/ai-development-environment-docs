...
- `/api/dsyms` and `/api/dsyms/uploads/*`
...
| `/api/public/crashes`                                                | `POST`        | Crash reports from shipped apps, rate limited         |
...
<Tip>
  To keep `/api/dsyms` and `/api/dsyms/uploads/*` behind Access instead of bypassing them, give the Access application a **Service Auth** policy for a service token, and send the token's `CF-Access-Client-Id` and `CF-Access-Client-Secret` headers from CI. The [Upload dSYMs action](/debugging/upload-dsyms-action#cloudflare-access) takes them in its `headers` input.
</Tip>
...
| `ARTIFACT_TRANSFER_DIRECTORY` | Durable staging for resumable cross-agent uploads. Defaults to the system temporary directory                |
| `ARTIFACT_TRANSFER_MAX_BYTES` | Maximum accepted transfer package size. Defaults to 20 GiB                                                   |
...
### Cross-agent artifact relay

Cross-agent runs use agent-authenticated routes under `/api/agent/build-artifact-transfers/{transferId}` for metadata, upload offset inspection, chunk append, completion, status, and ranged download. Keep these routes behind the same Cloudflare Access policy as the rest of the agent control plane; enrolled agents send their saved bearer credential and any configured Access service-token headers.

Uploads are sequential, resumable 16 MiB requests with validated `Upload-Offset` and `Upload-Length` headers. This keeps every request below common reverse-proxy upload limits, including Cloudflare's Free/Pro 100 MB limit. Downloads also use 16 MiB ranges for retry and resume reliability. The server verifies the final SHA-256 before placing the package in its bounded cache, and the target verifies it again before extracting into isolated temporary storage.

Give both the cache and transfer directories enough persistent space for your largest runnable apps. Active transfers are not pruned; completed packages use the cache ceiling and can be reused for later target agents. A server restart reconciles the saved offset with the staging file instead of retransmitting completed chunks.

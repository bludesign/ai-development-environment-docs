...
The production server accepts the standard Next.js `HOSTNAME` and `PORT` environment variables plus `DATABASE_URL`. Set `BETTER_AUTH_URL` to the same public origin users and the OIDC provider use.
...
- Set and back up a stable `BETTER_AUTH_SECRET`, and make `BETTER_AUTH_URL` match the public origin.

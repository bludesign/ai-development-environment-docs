...
| **Running** | **Reserve the worktree for this workflow** | Wait for earlier work, then keep other workflows, Plans, and Sessions off the worktree until this run finishes |
...
<Note>
  Worktree reservation is off by default and only applies when a top-level run resolves a worktree. It is a different thing from the overlap policy, which only counts this workflow's own runs: a reservation also holds back *other* workflows, Plans, and Sessions. The run waits behind earlier work, then holds the worktree until it succeeds, fails, or is cancelled. Plans and Sessions started by that workflow can still run inside the reservation.
</Note>
...
<Note>
  Turning off completion notifications suppresses successful-run notifications only. Failure and attention notifications still fire. MCP clients can set the same flag as `completionNotificationsEnabled` on `create_workflow` or `update_workflow_settings`, and pick the queue scope with `overlapScope` — `WORKTREE` or `GLOBAL` — on either tool.
</Note>

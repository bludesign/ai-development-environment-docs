...
A queued run shows a live **Queue** card before the run graph. It displays the combined workflow, Plan, and Session admission order for the run's worktree. The current workflow run is highlighted, and exclusive workflow runs carry an **Exclusive** badge.
...
The published graph, annotated with live state — which node succeeded, which failed, which is waiting, which is currently active. It updates as the run progresses.
...
<Steps>
  ...
  <Step title="Prepare the replay" icon="eye">
    **Prepare replay** computes a preview: how many descendant steps will re-run, and which external effects the original pass already caused.
  </Step>
...
<Tabs>
  <Tab title="Timeline">
    Every event the run has emitted, in order. Event types include run lifecycle events — started, pausing, paused, resumed, cancelled, succeeded, failed, data repaired, replayed — and step events: succeeded, failed, blocked, waiting, retry scheduled, recovery blocked, and waiting for codebase.
  </Tab>
  <Tab title="Step attempts">
    One row per attempt with **Step**, **Status**, **Generation**, **Iteration**, **Attempt**, **Duration**, and **Error**. Iteration distinguishes passes through a **For each** body; attempt distinguishes retries of the same step.
